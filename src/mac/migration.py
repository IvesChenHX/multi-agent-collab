...
def scan_v5(repo: Path) -> dict[str, Any]:
    ...
    for entry_index, source_entry in enumerate(entries):
        ...
        sources = [{
            "kind": "registry_entry",
            "path": "tasks/index.yaml",
            "pointer": f"/tasks/{entry_index}",
            "digest": _entity_digest(source_entry),
        }]
        if detail_present and detail is not None:
            sources.append({
                "kind": "task_detail",
        ...
        rows.append({
            ...
            "mapped_state": mapped_state,
            "status_class": "manual" if entry.get("status") == "blocked" else "automatic",
        ...
        for owner, definition in (ownership_data.get("owners") or {}).items():
            ...
            for pattern in definition.get("include", []):
                ...
                if match.status == "ambiguous":
                    ownership_ambiguities.append({
                        "path": probe, "owners": list(match.owners),
                    })
    except Exception as exc:
        ownership_data = {}
        warnings.append(f".agents/ownership.yaml is invalid: {type(exc).__name__}")
    ownership_ambiguities = sorted(
        {
            json.dumps(item, ensure_ascii=False, sort_keys=True): item
            for item in ownership_ambiguities
        }.values(),
        key=lambda item: (
            str(item.get("path", "")),
            tuple(str(owner) for owner in item.get("owners", [])),
        ),
    )
    ...
    for row in rows:
        ...
        status_inventory[status] = status_inventory.get(status, 0) + 1
    rollback_operations = [
        {
            "mutation": f"generate v6 entities from {item['path']}",
            "rollback_action": "git revert the migration commit; never delete v5 inputs",
            "verification": f"source digest remains {item['digest']}",
        }
        for item in source_documents
        if item.get("digest")
    ]
...
def convert_v5(
    ...
    for scanned in report["tasks"]:
        ...
        if legacy_id in existing:
            ...
            actions.append({
                ...
                "action": "unchanged",
                "source_entities": list(scanned["sources"]),
            })
            continue
    ...
    for item in actions:
        ...
        if item["action"] == "created":
            ...
            for path in sorted(child for child in task_dir.rglob("*") if child.is_file()):
    ...
    return {
        ...
        "unverifiable_evidence_created": 0,
        "generated_entities": generated_entities,
        "rollback": rollback,
    }
