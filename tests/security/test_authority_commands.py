...
def test_authority_sensitive_cli_mutations_all_fail_closed_without_runtime_verifier(
    ...
    disable_test_mutation_broker(monkeypatch)
    commands = [
        lambda: task_transition(task_id, "ready", 4, "transition-without-verifier", "governance-owner", [], None, None, tmp_path, True),
        lambda: task_cancel(task_id, 4, "cancel-without-verifier", "governance-owner", tmp_path, True),
        lambda: task_supersede(task_id, successor_id, 4, "supersede-without-verifier", "governance-owner", tmp_path, True),
        lambda: finding_waive(task_id, finding_id, "risk", ["control"], "2099-01-01T00:00:00Z", 4, "risk-without-verifier", "governance-owner", tmp_path, True),
        lambda: approval_record(task_id, "close", "approved", "HEAD", "governance-owner", "L1", 4, "approval-without-verifier", tmp_path, True),
        lambda: run_register(task_id, work_unit_id, "local-single", "context", None, None, tmp_path, None, "reviewer", "agent", "L2", 4, "run-without-verifier", tmp_path, True),
    ]
