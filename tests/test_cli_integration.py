...
def test_run_register_rejects_an_independent_repository(tmp_path: Path) -> None:
    ...
    scope_approve(
        task_id=task_id,
        expected_revision=0,
        idempotency_key="approve-run-binding-scope",
        actor="governance-owner",
        independence_level="L1",
        repo=root,
        json_output=True,
...
def test_run_finish_records_a_replayable_terminal_run_snapshot(
    ...
    signed_time = finish_event["payload"]["authority"]["issued_at"]
    assert finish_event["occurred_at"] == signed_time
    assert run["finished_at"] == signed_time
    assert signed_time != "2099-01-01T00:00:00Z"
